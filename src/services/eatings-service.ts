...
class EatingsService {
  async fetchDailyEatings(userId: string, day: string): Promise<DailyEatings> {
  ...
  async updateEating(eating: DishPortion): Promise<DishPortion> {
    const eatingFoodValue = calculateFoodValueForPortion(eating);
  ...
  async deleteEating(eating: DishPortion) {
    await supabase.from("eatings").delete().eq("id", eating.id!);
  }
}

export const eatingsService = new EatingsService();
